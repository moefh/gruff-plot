...
impl Token {
    ...
    fn number(n: f64, line: u32) -> Token {
        Token {
    ...
    pub fn read(&mut self) -> Result<Token> {
        loop {
            ...
            if ch.is_ascii_digit() {
                let mut num = (ch as u32 - '0' as u32) as f64;
                let mut dot = 0;
                loop {
                    match self.next_char() {
                        Some(c) if c.is_ascii_digit() => {
                            ...
                            if dot > 0 {
                                num += digit * 10_f64.powf(-dot as f64);
                                dot += 1;
                            } else {
                                num = num * 10.0 + digit;
                            }
                ...
                return Ok(Token::number(num, self.line));
            }
